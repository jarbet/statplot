...
test_that("returns a ggplot with default arguments", {
    p <- plot_two_bar_comparison(make_df(), y_label = "Score")
    expect_s3_class(p, "ggplot")
})

test_that("returns a ggplot with error_direction = 'up'", {
    p <- plot_two_bar_comparison(make_df(), error_direction = "up")
    expect_s3_class(p, "ggplot")
})

test_that("returns a ggplot with facet = TRUE", {
    p <- plot_two_bar_comparison(make_df(), facet = TRUE)
    expect_s3_class(p, "ggplot")
})

test_that("returns a ggplot with facet = TRUE and strip_position = 'bottom'", {
    p <- plot_two_bar_comparison(
        make_df(),
        facet = TRUE,
...
test_that("returns a ggplot when p_col = NULL (no brackets)", {
    p <- plot_two_bar_comparison(make_df(), p_col = NULL)
    expect_s3_class(p, "ggplot")
})
...
test_that("errors when required column is missing", {
    df <- make_df()
    expect_error(
        plot_two_bar_comparison(df, mean_col = "nonexistent"),
        "Column\\(s\\) not found"
    )
})
...
test_that("errors when error_direction is invalid", {
    expect_error(
        plot_two_bar_comparison(make_df(), error_direction = "down")
    )
})
...
test_that("errors when df is not a data.frame", {
    expect_error(plot_two_bar_comparison(list(a = 1)))
})

# ── Error bar layers ───────────────────────────────────────────────────────────
test_that("error_direction = 'both' uses geom_errorbar", {
    p <- plot_two_bar_comparison(make_df(), error_direction = "both")
...
test_that("significance brackets appear when p < p_cutoff", {
    p <- plot_two_bar_comparison(make_df(), p_cutoff = 0.05)
    layer_classes <- vapply(p$layers, \(l) class(l$geom)[1], character(1))
...
test_that("no brackets when all p-values exceed p_cutoff", {
    df <- make_df()
    df$p_value <- 0.9
    p <- plot_two_bar_comparison(df, p_cutoff = 0.05)
    layer_classes <- vapply(p$layers, \(l) class(l$geom)[1], character(1))
...
test_that("custom label_col is used in bracket text", {
    ...
    p <- plot_two_bar_comparison(df, label_col = "label", p_cutoff = 0.05)
    layer_classes <- vapply(p$layers, \(l) class(l$geom)[1], character(1))
...
test_that("facet = FALSE uses FacetWrap", {
    p <- plot_two_bar_comparison(make_df(), facet = FALSE)
...
test_that("custom group_col and condition_col names are respected", {
    ...
    p <- plot_two_bar_comparison(df, group_col = "grp", condition_col = "cond")
    expect_s3_class(p, "ggplot")
})
